...
export async function findPatientByPhone(phone: string): Promise<IPatientBasic | null> {
  // ตัวอย่าง endpoint: /api/patients/search?phone=08xxxxxxxx
  const res = await axios.get(`/api/patients/search`, { params: { phone } });
  // ควรให้ BE คืน 404 เมื่อไม่พบ -> try/catch ตรงนี้ก็ได้
