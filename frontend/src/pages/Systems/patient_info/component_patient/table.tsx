...
import { fetchPatients, deletePatient, Patient } from "../../../../services/Patient/patient";
...
// Define PatientRow interface for table display
...
const toRows = (resp: any): PatientRow[] => {
  ...
  return raw.map((p: any, i: number) => ({
    ...
    lastname: p.last_name ?? p.lastname ?? p.Lastname ?? "",
...
const PatienTable: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const r = await fetchPatients(); // Returns { data: Patient[], total: number }
      console.log(
