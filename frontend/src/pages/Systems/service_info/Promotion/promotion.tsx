...
import { 
  fetchServices,
  fetchPromotions,
  createPromotion as createServicePromotion,
  updatePromotion,
  deletePromotion,
...
const Promotion = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // แปลงข้อมูลจาก ServicePromotion เป็น Promotion interface
                const promotionList = promotionResponse.data.map((p: ServicePromotion) => ({
                    id: p.id,
                    name_promotion: p.title,
                    service_id: p.service_ids[0] || 1, // ใช้ service แรกจาก array
                    promotion_detail: p.description,
                    cost: p.discount_value,
                    date_start: p.start_date,
                    date_end: p.end_date
                }));
                
                // แปลงข้อมูลจาก DentalService
                const serviceList = serviceResponse.data.map((s: DentalService) => ({
                    id: s.id,
                ...
                setItems(promotionList); // สำหรับ edit/delete/add
            } catch (error) {
    ...
    const handleModalSave = async () => {
        if (modal.type === 'add') {
            ...
            try {
                // แปลงเป็น CreatePromotionPayload format
                const payload: CreatePromotionPayload = {
                    promotion_code: `PROMO${Date.now()}`,
                    title: newItem.name_promotion,
                    description: newItem.promotion_detail,
                    service_ids: [newItem.service_id!],
                    discount_type: 'fixed_amount',
                    discount_value: newItem.cost,
                    start_date: newItem.date_start,
                    end_date: newItem.date_end
                };

                const createdPromotion = await createServicePromotion(payload);
                
                // แปลงกลับเป็น Promotion format
                const savedPromotion: Promotion = {
                    id: createdPromotion.id,
                    name_promotion: createdPromotion.title,
                    service_id: createdPromotion.service_ids[0],
                    promotion_detail: createdPromotion.description,
                    cost: createdPromotion.discount_value,
                    date_start: createdPromotion.start_date,
                    date_end: createdPromotion.end_date
                };
            ...
            try {
                ...
                const payload: Partial<CreatePromotionPayload> = {
                    title: updatedItem.name_promotion,
                    description: updatedItem.promotion_detail,
                    service_ids: [updatedItem.service_id!],
                    discount_type: 'fixed_amount',
                    discount_value: updatedItem.cost,
                    start_date: updatedItem.date_start,
                    end_date: updatedItem.date_end
                };

                const updatedServicePromotion = await updatePromotion(updatedItem.id!, payload);
                
                // แปลงกลับเป็น Promotion format
                const updatedPromotion: Promotion = {
                    id: updatedServicePromotion.id,
                    name_promotion: updatedServicePromotion.title,
                    service_id: updatedServicePromotion.service_ids[0],
                    promotion_detail: updatedServicePromotion.description,
                    cost: updatedServicePromotion.discount_value,
                    date_start: updatedServicePromotion.start_date,
                    date_end: updatedServicePromotion.end_date
                };
    ...
    const handleSave = async (index: number) => {
        ...
        try {
            // แปลงเป็น CreatePromotionPayload format
            const payload: Partial<CreatePromotionPayload> = {
                title: itemToUpdate.name_promotion,
                description: itemToUpdate.promotion_detail,
                service_ids: [itemToUpdate.service_id!],
                discount_type: 'fixed_amount',
                discount_value: itemToUpdate.cost,
                start_date: itemToUpdate.date_start,
                end_date: itemToUpdate.date_end
            };

            const updatedServicePromotion = await updatePromotion(itemToUpdate.id!, payload);
